...
{
    public partial class CadastroFechamentoContato : Page
    {
        ...
        {
            using (var repository = new Repository<FechamentoContato>(new Context<FechamentoContato>()))
            {
                var datasource = new List<FechamentoContato>();
                switch (type)
                {
                    case 1: datasource.AddRange(repository.All().OrderBy(p => p.FechCodigo)); break;
                    case 2: datasource.AddRange(repository.All().Where(p => p.FechDescricao.ToLower().Contains(pesquisa.Text.Trim().ToLower())).OrderBy(p => p.FechDescricao)); break;
                }
        ...
        private string GeraSql()
        {
            ...
            string sqlupdate = "UPDATE CA_fechamentosContatos SET FechDescricao = '" + TBNome.Text + "' WHERE  FechCodigo = '" + Session["AlrteraCodigo_modelo"] + "' ";
            var sqlinsert = "INSERT INTO CA_fechamentosContatos( FechDescricao  ) VALUES( '" + TBNome.Text + "' ) ";
        ...
        protected void GridView1_SelectedIndexChanged(object sender, EventArgs e)
        {
            using (var bd = new DC_ProtocoloAgilDataContext(GetConfig.Config()))
            {
                ...
                var dados = from i in bd.CA_fechamentosContatos 
                            where i.FechCodigo.Equals(WebUtility.HtmlDecode(row.Cells[0].Text))
                            select new { i.FechCodigo, i.FechDescricao };
                if(dados.Count() > 0)
                {
                    var tipo = dados.First();
                    TBcodigo.Text = tipo.FechCodigo.ToString();
                    Session["AlrteraCodigo_modelo"] = tipo.FechCodigo.ToString();
                    TBcodigo.Enabled = false;
                    TBNome.Text = tipo.FechDescricao;
