...
{
    public partial class CadastroTiposContato : Page
    {
        ...
        {
            using (var repository = new Repository<TipoContatos>(new Context<TipoContatos>()))
            {
                var datasource = new List<TipoContatos>();
                switch (type)
                {
                    case 1: datasource.AddRange(repository.All().OrderBy(p => p.Tco_Codigo)); break;
                    case 2: datasource.AddRange(repository.All().Where(p => p.Tco_Descricao.ToLower().Contains(pesquisa.Text.Trim().ToLower())).OrderBy(p => p.Tco_Descricao)); break;
                }
        ...
        private string GeraSql()
        {
            ...
            string sqlupdate = "UPDATE CA_TiposContatos SET Tco_Descricao = '" + TBNome.Text + "' WHERE  Tco_Codigo = '" + Session["AlrteraCodigo_modelo"] + "' ";
            var sqlinsert = "INSERT INTO CA_TiposContatos( Tco_Descricao  ) VALUES( '" + TBNome.Text + "' ) ";
        ...
        protected void GridView1_SelectedIndexChanged(object sender, EventArgs e)
        {
            using (var bd = new DC_ProtocoloAgilDataContext(GetConfig.Config()))
            {
                ...
                var dados = from i in bd.CA_TiposContatos 
                            where i.Tco_Codigo.Equals(WebUtility.HtmlDecode(row.Cells[0].Text))
                            select new { i.Tco_Codigo, i.Tco_Descricao };
                if(dados.Count() > 0)
                {
                    var tipo = dados.First();
                    TBcodigo.Text = tipo.Tco_Codigo.ToString();
                    Session["AlrteraCodigo_modelo"] = tipo.Tco_Codigo.ToString();
                    TBcodigo.Enabled = false;
                    TBNome.Text = tipo.Tco_Descricao;
