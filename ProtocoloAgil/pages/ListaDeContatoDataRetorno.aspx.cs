...
{
    public partial class ListaDeContatoDataRetorno : Page
    {
        ...
        private void BindGridView()
        {
            ...
            {
                where += " and C.CocPrevisaoRetorno >= '" + txtDataInicio.Text + "' and C.CocPrevisaoRetorno <= '" + txtDataTermino.Text + "'";
            }

            var sql = "Select cocDataContato, T.Tco_Descricao, F.FechDescricao, C.CocUsuarioContato, C.CocDescricaoContato, C.CocResultadoContato, CC.CacNome, C.CocPrevisaoRetorno, S.StcCodigo   from CA_contatos C   left join CA_TiposContatos T on C.CocTipo = T.Tco_Codigo  left join CA_fechamentosContatos F on C.CocCodigoFechamento = F.FechCodigo left join CA_CadastroClientes CC on C.CocCliente = CC.CacCodigo left join CA_StatusCliente S on CC.CacStatus = S.StcCodigo  where 1 = 1 " + where + "";
